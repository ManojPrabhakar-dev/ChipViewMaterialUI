...
namespace ChipViewApp
{
    ...
    public class DynamicTimingDiagram
    {
        ...
        public Timing_Parameters GetPathData(Timing_Parameters timingParam)
        {
            try
            {
                timingParam.INTEGSEQUENCE_DATA = Geometry.Parse(GenerateTimingData(TIMING_TYPE.INTEG_SEQUENCE, timingParam));

                timingParam.LED_DATA = Geometry.Parse(GenerateTimingData(TIMING_TYPE.LED, timingParam));

                timingParam.MODULATESTIMULUS_DATA = Geometry.Parse(GenerateTimingData(TIMING_TYPE.MODULATED_STIMULUS, timingParam));

                timingParam.PRECONDITION_DATA = Geometry.Parse(GenerateTimingData(TIMING_TYPE.PRE_CONDITION, timingParam));
            }
            catch (Exception ex)
            {
        ...
        public Timing_Parameters GetPathData1(Timing_Parameters timingParam)
        {
            try
            {

                var ledLength = GetTimingTotalLength(TIMING_TYPE.LED,timingParam);

                var integ_length = GetTimingTotalLength(TIMING_TYPE.INTEG_SEQUENCE,timingParam);

                Console.WriteLine("LedLength = " + ledLength.timingCheckpoint + " integ_length = " + integ_length.timingCheckpoint);

                if (ledLength.timingCheckpoint > integ_length.timingCheckpoint)
                {
                    var ref_timecoord = new TimeCoord(TIMING_TYPE.LED);
                    timingParam.LED_DATA = Geometry.Parse(GenerateTimingData1(TIMING_TYPE.LED, timingParam, ref_timecoord));

                    timingParam.INTEGSEQUENCE_DATA = Geometry.Parse(GenerateTimingData1(TIMING_TYPE.INTEG_SEQUENCE, timingParam, ref_timecoord));

                    timingParam.PRECONDITION_DATA = Geometry.Parse(GenerateTimingData1(TIMING_TYPE.PRE_CONDITION, timingParam, ref_timecoord));
        ...
        public Timing_Parameters GetPathData2(Timing_Parameters timingParam)
        {
            try
            {
                ...
                if ((led_length.timingCheckpoint >= integ_length.timingCheckpoint) && (led_length.timingCheckpoint >= mod_length.timingCheckpoint))
                {
                    ...
                    timingParam.MODULATESTIMULUS_DATA = Geometry.Parse(GenerateTimingData1(TIMING_TYPE.MODULATED_STIMULUS, timingParam, ref_timecoord));
        ...
        private string GenerateTimingData(TIMING_TYPE eTimingType, Timing_Parameters timingParam, bool reinit_endwidth = false)
        {
            ...
            try
            {
                ...
                else if (eTimingType == TIMING_TYPE.LED)
                {
                    ...
                    if (X4 > END_WIDTH)
                    {
                        ...
                        GenerateTimingData(TIMING_TYPE.INTEG_SEQUENCE, timingParam, true);
                    }
                    else
                    {
        ...
        private string GenerateTimingData1(TIMING_TYPE eTimingType, Timing_Parameters timingParam, TimeCoord timeCoord)
        {
            string timingData = string.Empty;
            ...
            try
            {
                ...
                else if (eTimingType == TIMING_TYPE.LED)
                {
                    ...
                    //    // GenerateTimingData1(TIMING_TYPE.INTEG_SEQUENCE, timingParam, true);
                    //}
                    //else
                    //{
                    //    end_width_diff = END_WIDTH - X4;
                ...
                else if (eTimingType == TIMING_TYPE.MODULATED_STIMULUS)
                {
                    ...
                    //    // GenerateTimingData1(TIMING_TYPE.MODULATED_STIMULUS, timingParam, true);
                    //}
                    //else
                    //{
                    //    end_width_diff = END_WIDTH - X4;
